...
function post(subtitle,user,url){
  return `
...
let postNode = document.querySelector('#posts');

addEventListener('load', (event) => {
  console.log("hello")
  fetch('https://rssh-ub-pi-ten.vercel.app/picnob/user/wannabecountryboi4208')
    .then((response) => response.text())
    .then((str) => new window.DOMParser().parseFromString(str, 'text/xml'))
    .then((data) => {
      var items = data.querySelectorAll("channel > item")
      for (item of items){
        console.log(item.querySelector("img").innerHTML)
        postNode.innerHTML += item.querySelector("img").innerHTML;
      }
      console.log(items)
    });
});
